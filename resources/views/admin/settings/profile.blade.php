...
<head>
    ...
    <title>Edit Profil Admin - TPQ Digital</title>
...
<body
    ...
    <div
        ...
        <main class="flex flex-col gap-6 px-5 pt-6 flex-1 pb-32">
            ...
            @if(session('success'))
            <div
                class="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl text-green-600 dark:text-green-400 text-sm text-center flex items-center justify-center gap-2">
            ...
            @if($errors->any())
            <div
                class="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl text-red-600 dark:text-red-400 text-sm text-center">
                {{ $errors->first() }}
            </div>
            @endif
            ...
            <!-- Form -->
            <form id="profileForm" action="/admin/settings/profile/update" method="POST" enctype="multipart/form-data"
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="relative">
                        ...
                        <input name="no_hp"
                            class="w-full rounded-2xl border border-gray-200 bg-white py-3.5 pl-11 pr-4 text-sm font-medium text-gray-900 focus:border-primary focus:ring-primary dark:border-gray-700 dark:bg-gray-800 dark:text-white placeholder-gray-400 shadow-sm"
                            placeholder="Contoh: 0812..." type="tel" value="{{ session('user.no_hp', '') }}" />
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-2">
                    <label
                        class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider ml-1">Alamat</label>
                    <div class="relative">
                        <span class="material-symbols-outlined absolute left-4 top-4 text-gray-400"
                            style="font-size: 20px;">home</span>
                        <textarea name="alamat" rows="3"
                            class="w-full rounded-2xl border border-gray-200 bg-white py-3.5 pl-11 pr-4 text-sm font-medium text-gray-900 focus:border-primary focus:ring-primary dark:border-gray-700 dark:bg-gray-800 dark:text-white placeholder-gray-400 shadow-sm resize-none"
                            placeholder="Masukkan alamat lengkap">{{ session('user.alamat', '') }}</textarea>
                    </div>
                </div>
