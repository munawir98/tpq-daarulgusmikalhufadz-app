...
<body class="bg-background-light dark:bg-background-dark text-[#0e141b] dark:text-slate-100 min-h-screen flex flex-col">
    ...
    <header
        ...
        <div class="flex items-center gap-3">
            ...
            <h1 class="text-[#0e141b] dark:text-white text-lg font-bold leading-tight">Tambah Kegiatan Ekskul</h1>
        </div>
    ...
    <form action="{{ route('ustadz.laporan.ekskul.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <main class="flex-1 overflow-y-auto px-4 py-6 max-w-md mx-auto w-full">
            <div class="space-y-6">
                ...
                <!-- Activity Name -->
                <div class="flex flex-col gap-2">
                    <label class="text-[#0e141b] dark:text-slate-200 text-sm font-semibold px-1">Nama Kegiatan</label>
                    <input name="nama" type="text" value="{{ old('nama') }}" required
                        ...
                        placeholder="Contoh: Latihan Tahfidz, Pramuka..." />
                    @error('nama')
                    <p class="text-red-500 text-xs px-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Pelatih -->
                <div class="flex flex-col gap-2">
                    <label class="text-[#0e141b] dark:text-slate-200 text-sm font-semibold px-1">Nama Pelatih
                        (Opsional)</label>
                    <input name="pelatih" type="text" value="{{ old('pelatih') }}"
                        class="form-input w-full rounded-lg text-[#0e141b] dark:text-white border border-[#d0dbe7] dark:border-slate-700 bg-white dark:bg-slate-900 focus:border-primary focus:ring-1 focus:ring-primary h-14 p-[15px] text-base font-normal"
                        placeholder="Nama pelatih atau pembimbing..." />
                </div>

                <!-- Jumlah Peserta -->
                <div class="flex flex-col gap-2">
                    <label class="text-[#0e141b] dark:text-slate-200 text-sm font-semibold px-1">Jumlah Peserta</label>
                    <input name="jumlah_peserta" type="number" value="{{ old('jumlah_peserta', 0) }}" min="0"
                        class="form-input w-full rounded-lg text-[#0e141b] dark:text-white border border-[#d0dbe7] dark:border-slate-700 bg-white dark:bg-slate-900 focus:border-primary focus:ring-1 focus:ring-primary h-14 p-[15px] text-base font-normal"
                        placeholder="0" />
                </div>
                ...
                <!-- Keterangan -->
                <div class="flex flex-col gap-2">
                    <label class="text-[#0e141b] dark:text-slate-200 text-sm font-semibold px-1">Keterangan</label>
                    <textarea name="keterangan"
                        ...
                        placeholder="Catatan atau keterangan tambahan tentang kegiatan...">{{ old('keterangan') }}</textarea>
                </div>
